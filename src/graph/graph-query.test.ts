...
describe("queryGraph - vulnerable filter values", () => {
  ...
  it("vulnerable=false returns only complete routes with no vulnerable nodes", () => {
    const graph = createExtendedFilterGraph();
...
describe("queryGraph - two-filter combinations", () => {
  ...
  it("publicExposed=true + vulnerable=false returns empty when every public route hits a vuln", () => {
    const graph = createExtendedFilterGraph();

    const result = queryGraph(graph, { publicExposed: true, vulnerable: false });
